...
export interface AssistTimelineInput {
...
export interface AssistTimelineOptions {
...
async function resolveAssistModel(vaultRoot: string, options: AssistTimelineOptions): Promise<ResolvedAssistModel> {
...
function normalizeFields(input: unknown): AssistTimelineField[] {
  if (!Array.isArray(input)) {
    return [];
  }

  const fields: AssistTimelineField[] = [];
...
function createFieldBatches(fields: AssistTimelineField[], template?: string): AssistBatch[] {
  if (template !== "detailed" && fields.length <= BATCH_FIELD_THRESHOLD) {
    return [{ name: "全部字段", fields }];
  }

  const byGroup = new Map<string, AssistTimelineField[]>();
...
function buildMessages(input: Required<Pick<AssistTimelineInput, "mode" | "userPrompt">> & {
  projectId?: string;
  template?: string;
  line?: string;
  eventName?: string;
  ...
  return [
    {
      ...
      content: [
        ...
        "字段内容使用中文，保持叙事逻辑、事件因果和人物/世界状态前后一致。",
        input.mode === "fanfic"
          ? "同人模式：根据用户提供的事件名和来源作品，用你已知的公开剧情资料填写；不确定或不知道的字段留空，不要编造。"
          : "原创模式：根据用户的事件方向，为该线类型生成贴切的事件内容，包括起因、经过、结果、转折和影响，让事件可写、清晰、能承接剧情。",
      ].join("\n"),
    },
    {
      role: "user",
      content: [
        ...
        `线类型：${input.line || "未指定"}`,
        input.mode === "fanfic" ? `事件名：${input.eventName || "未指定"}` : "",
...
function normalizeResultFields(raw: string, fields: AssistTimelineField[]): Record<string, string> {
...
async function generateFieldBatch(
  registry: ProviderRegistry,
  modelId: string,
  input: Required<Pick<AssistTimelineInput, "mode" | "userPrompt">> & {
    projectId?: string;
    template?: string;
    line?: string;
    eventName?: string;
  ...
  for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (isAsyncIterable(response)) {
        throw new Error("模型返回了流式响应，时间线辅助填充接口期望完整 JSON 响应");
      }
...
export async function assistTimeline(
  vaultRoot: string,
  input: AssistTimelineInput,
  options: AssistTimelineOptions,
): Promise<AssistTimelineResponse> {
  const mode = input.mode === "fanfic" ? "fanfic" : "original";
  const userPrompt = typeof input.userPrompt === "string" ? input.userPrompt.trim() : "";
  if (!userPrompt) {
    throw createHttpError(400, "ASSIST_TIMELINE_PROMPT_REQUIRED", "请先填写时间线事件描述");
  }

  const fields = normalizeFields(input.fields);
  if (!fields.length) {
    throw createHttpError(400, "ASSIST_TIMELINE_FIELDS_REQUIRED", "没有可填写的时间线字段");
  }
  ...
  try {
    for (const batch of batches) {
      const batchResult = await generateFieldBatch(registry, modelId, {
        ...
        line: input.line,
        eventName: typeof input.eventName === "string" ? input.eventName.trim() : "",
    ...
    throw createHttpError(
      502,
      "ASSIST_TIMELINE_FAILED",
      `时间线 AI 辅助填充失败：${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
